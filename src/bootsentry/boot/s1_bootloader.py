...
def run_stage_1(
    ...
    try:
        ...
        h = BootHandoff(
            ...
            event_log_data=[],
            status="HALTED",
            error_message=f"Inter-stage handoff security verification failed in S1: {e}",
        )
        h.save(run_path / "handoff_s1.json")
        return h
    ...
    if spawn_next:
        ...
        try:
            ...
            handoff.status = "HALTED"
            handoff.error_message = "S2 process timed out after 30s"
            handoff.save(out_file)
