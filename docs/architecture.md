...
$$\text{Is Component } C_i \text{ authentically signed by Root of Trust } K_{root}^{pub}?$$
...
3. In-memory tampering or unauthorized service execution order.
4. Subtle performance degradation or multi-boot operational drift.

**BootSentry** implements a **Dual-Assurance Security Model**:
- **Static Gate Assurance (Gates 1 & 2)**: Deterministic, fail-closed post-quantum cryptographic signature (ML-DSA-65) and TPM measured boot checks.
...
BootSentry decomposes boot execution into 4 distinct stages. In this Python reference framework, stages execute sequentially with authenticated HMAC-SHA256 `BootHandoff` tokens passing verifiable state between stage boundaries:
...
- **Responsibilities**:
  ...
  5. Passes verified execution handoff to Stage 1 via authenticated state token.

### Stage 1: Bootloader (`s1_bootloader.py`)
- **Responsibilities**:
  1. Validates authenticated handoff token from S0 and checks event-log consistency.
  ...
  5. Passes authenticated handoff token to Stage 2.

### Stage 2: Kernel (`s2_kernel.py`)
- **Responsibilities**:
  1. Validates authenticated handoff token from S1 and checks event-log consistency.
  ...
  5. Passes authenticated handoff token to Stage 3.

### Stage 3: Init & Computational Services (`s3_init.py` & `services.py`)
- **Responsibilities**:
  1. Validates authenticated handoff token from S2 and checks event-log consistency.
  2. Launches user-space services in deterministic sequence: `[svc_a, svc_b, svc_c, svc_attest, svc_e]`. Services in this prototype are modular Python functions executing real computational workloads (prime sieves, matrix permutations, SHA-256 trees).
  3. Extends $\text{PCR}[3]$ for each individual service payload.
  4. Generates signed post-quantum Attestation Quote ($\text{PCR}[0..3]$ + Nonce + Event Log digest + ISO-8601 UTC timestamp).
...
Manifests are serialized using RFC 8785 canonical JSON formatting (strictly sorted keys, whitespace-free, standard float representation) to ensure bit-exact signature verification:
...
3. **Memory Footprint Features** (5 features): Peak Resident Set Size ($\text{RSS}_{peak}$), per-stage RSS ($\text{RSS}_{S0..S3}$).
4. **Context Switching & System Dynamics** (7 features): Voluntary context switches, involuntary context switches, voluntary switch ratio ($\frac{\text{vol}}{\text{vol}+\text{invol}}$), page faults minor/major, and storage I/O bytes read/written.
5. **CPU Consumption Features** (2 features): User mode CPU time ($t_{user}$) and kernel mode CPU time ($t_{sys}$).
...
- Computes the average Negative Log-Likelihood ($\text{NLL}$) and flags structural reordering attacks (Attack A3).

### Layer 3: EWMA / CUSUM Multi-Boot Drift Monitor
- Tracks multi-boot parameter shifts relative to a calibrated warmup window:
$$z_t = \frac{x_t - \text{median}_{ref}}{\text{scale}_{ref}}$$
- Runs a Cumulative Sum (CUSUM) detector:
$$S_t^+ = \max(0, S_{t-1}^+ + z_t - k)$$
- Triggers when $S_t^+ > h$, detecting stealth accumulation across boots (Attack A4).
...
Enforces hard security invariants:
1. `RULE_SVN_ROLLBACK`: Observed $\text{SVN} < \text{MinTrustedSVN}$
2. `RULE_PCR_NOT_ALLOWLISTED`: Final PCR state not present in golden allowlist (when allowlist configured)
3. `RULE_STAGE_MISMATCH`: Execution sequence violates $S0 \to S1 \to S2 \to S3$
